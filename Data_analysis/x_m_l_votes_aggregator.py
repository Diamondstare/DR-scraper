...
OUTPUT_SVG = Path("question2_vs_log_votes.svg")
...
def fit_trend_line(points: list[Point]) -> tuple[float, float] | None:
    """
    Fit y = a*x + b using least squares.
    Returns (a, b), or None if not enough data.
    """
    if len(points) < 2:
        return None
...
"""
    ...
    lines.append(
        ...
        f'Question 3 answer vs log10(votes + 1)'
        f"</text>"
    )
    ...
    lines.append(
        ...
        f'font-size="16" font-family="Arial">Answer to question 3</text>'
    )
    lines.append(
    ...
    for i in range(min_x, max_x + 1):
        ...
        lines.append(
            f'<text x="{px}" y="{y2 + 22}" text-anchor="middle" font-size="12" font-family="Arial">{i}</text>'
        )

    y_ticks = 5
    for i in range(y_ticks + 1):
        value = (max_y / y_ticks) * i if y_ticks else 0
        py = y_to_px(value)
        lines.append(f'<line x1="{x0 - 6}" y1="{py}" x2="{x0}" y2="{py}" stroke="black"/>')
        lines.append(
            f'<text x="{x0 - 10}" y="{py + 4}" text-anchor="end" font-size="12" font-family="Arial">'
            f"{value:.1f}</text>"
        )

    # Trend line across all points
    fit = fit_trend_line(points)
    if fit is not None:
        slope, intercept = fit
        x_start = min_x
        x_end = max_x
        ...
        lines.append(
            f'<line x1="{x_to_px(x_start):.2f}" y1="{y_to_px(y_start):.2f}" '
            f'x2="{x_to_px(x_end):.2f}" y2="{y_to_px(y_end):.2f}" '
            f'stroke="black" stroke-width="3" opacity="0.8"/>'
        )
    ...
    for point in points:
        ...
        title = svg_escape(f"{point.label} ({point.party_code or 'none'}): q3={point.x}, y={point.y:.2f}")
